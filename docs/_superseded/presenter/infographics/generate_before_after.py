...
def draw_workflow_icon(ax, x, y, icon_type, color):
    ...
    if icon_type == 'search':
        # Magnifying glass
        circle = Circle((x, y), 0.15, facecolor='none',
                        edgecolor=color, linewidth=2)
        ax.add_patch(circle)
        ax.plot([x + 0.1, x + 0.25], [y - 0.1, y - 0.25],
...
def create_before_after_poster():
    ...
    before_steps = [
        ('1', 'Research Documentation', '15-30 min',
         'Searching Azure docs, Stack Overflow'),
        ('2', 'Find & Adapt Examples', '20-40 min', 'Copy-paste, trial and error'),
        ('3', 'Write Infrastructure Code', '30-60 min', 'Manual Bicep authoring'),
